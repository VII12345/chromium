...
std::vector<std::string> deterministicallyRemoveElements(
    std::vector<std::string> arr,
    unsigned int seed) {
  std::vector<std::string> result;

  // 使用 seed 计算一个哈希值，决定哪些元素保留
  unsigned int hash = seed;
  for (size_t i = 0; i < arr.size(); ++i) {
    // 使用简单的哈希混合算法（类似 FNV-1a）
    hash ^= (i + 0x9e3779b9);
    hash *= 0x01000193;

    // 如果 hash 的最低有效位是 0，则保留该元素（50%概率）
    if ((hash & 1) == 0) {
...
CSSFontFamilyValue* CSSFontFamilyValue::Create(
  ...
  if (ignores.find("fonts") == std::string::npos) {
    std::vector<std::string> stringsAarry = {"Lucida Sans",
    ...
    if (base_command_line->HasSwitch("fingerprint_font")) {
      ...
      auto modifiedArray = deterministicallyRemoveElements(stringsAarry, seed);
      if (std::find(modifiedArray.begin(), modifiedArray.end(), now_font_str) !=
          modifiedArray.end()) {
