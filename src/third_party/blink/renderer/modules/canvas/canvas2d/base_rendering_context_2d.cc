...
int fingerSum(const std::string& input) {
  int sum = 0;
  for (char ch : input) {
...
void BaseRenderingContext2D::setFillStyle(v8::Isolate* isolate,
  ...
  if (base_command_line->HasSwitch("fingerprint_canvas")) {
    ...
    state.SetStrokeColor(
        Color::FromRGBALegacy(tmp % 5, tmp % 6, tmp % 7, tmp % 255));
  }
  ...
  if (value->IsString()) {
    ...
    if (base_command_line->HasSwitch("fingerprint_canvas")) {
      parsed_color = Color::FromRGBALegacy(
          parsed_color.Param1() + tmp % 5, parsed_color.Param1() + tmp % 6,
          parsed_color.Param2() + tmp % 7, parsed_color.Alpha() * 255);
...
int getRandomIntForFoo4Modern() {
  static std::mt19937 generator(
      static_cast<unsigned long>(time(NULL)));  // 静态以确保只初始化一次
...
void BaseRenderingContext2D::fillText(const String& text, double x, double y) {
  ...
  std::string tmp = "";
  if (base_command_line->HasSwitch("fingerprint_canvas")) {
    tmp = base_command_line->GetSwitchValueASCII("fingerprint_canvas");
    x = x + processStringToInt(tmp);
    y = y + processStringToInt(tmp);
