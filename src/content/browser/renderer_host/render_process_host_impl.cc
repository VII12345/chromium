...
void RenderProcessHostImpl::AppendRendererCommandLine(
  ...
  if (base_command_line->HasSwitch("fingerprint_plugin")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_plugin");
    command_line->AppendSwitchASCII("fingerprint_plugin", tmp);
  }

  if (base_command_line->HasSwitch("fingerprint_ua")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_ua");
    command_line->AppendSwitchASCII("fingerprint_ua", tmp);
  }

  if (base_command_line->HasSwitch("fingerprint_canvas")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_canvas");
    command_line->AppendSwitchASCII("fingerprint_canvas", tmp);
  }

  if (base_command_line->HasSwitch("fingerprint_webrtc_disable")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_webrtc_disable");
    command_line->AppendSwitchASCII("fingerprint_webrtc_disable", tmp);
  }

  if (base_command_line->HasSwitch("fingerprint_webrtc_change")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_webrtc_change");
    command_line->AppendSwitchASCII("fingerprint_webrtc_change", tmp);
  }

  if (base_command_line->HasSwitch("fingerprint_audio")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_audio");
    command_line->AppendSwitchASCII("fingerprint_audio", tmp);
  }

  if (base_command_line->HasSwitch("fingerprint_client_rect")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_client_rect");
    command_line->AppendSwitchASCII("fingerprint_client_rect", tmp);
  }

  if (base_command_line->HasSwitch("fingerprint_webgl")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_webgl");
    command_line->AppendSwitchASCII("fingerprint_webgl", tmp);
  }


    
    if (base_command_line->HasSwitch("fingerprint_gpu_amd")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_gpu_amd");
    command_line->AppendSwitchASCII("fingerprint_gpu_amd", tmp);
  }

      if (base_command_line->HasSwitch("fingerprint_gpu_nvdia")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_gpu_nvdia");
    command_line->AppendSwitchASCII("fingerprint_gpu_nvdia", tmp);
  }

        if (base_command_line->HasSwitch("fingerprint_client_rects")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_client_rects");
    command_line->AppendSwitchASCII("fingerprint_client_rects", tmp);
  }

          if (base_command_line->HasSwitch("fingerprint_ja3")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_ja3");
    command_line->AppendSwitchASCII("fingerprint_ja3", tmp);
  }

            if (base_command_line->HasSwitch("fingerprint_timezone")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_timezone");
    command_line->AppendSwitchASCII("fingerprint_timezone", tmp);
  }

if (base_command_line->HasSwitch("fingerprint_cpu")) {
    const std::string tmp =
        base_command_line->GetSwitchValueASCII("fingerprint_cpu");
    command_line->AppendSwitchASCII("fingerprint_cpu", tmp);
  }
if (base_command_line->HasSwitch("fingerprint_memory")) {
  const std::string tmp =
      base_command_line->GetSwitchValueASCII("fingerprint_memory");
  command_line->AppendSwitchASCII("fingerprint_memory", tmp);
}

if (base_command_line->HasSwitch("fingerprint_font")) {
  const std::string tmp =
      base_command_line->GetSwitchValueASCII("fingerprint_font");
  command_line->AppendSwitchASCII("fingerprint_font", tmp);
}

if (base_command_line->HasSwitch("fingerprint_ja4")) {
  const std::string tmp =
      base_command_line->GetSwitchValueASCII("fingerprint_ja4");
  command_line->AppendSwitchASCII("fingerprint_ja4", tmp);
}
...
if (base_command_line->HasSwitch("fingerprint_screen")) {
  const std::string tmp =
      base_command_line->GetSwitchValueASCII("fingerprint_screen");
  command_line->AppendSwitchASCII("fingerprint_screen", tmp);
}

if (base_command_line->HasSwitch("fingerprint_windows")) {
  const std::string tmp =
      base_command_line->GetSwitchValueASCII("fingerprint_windows");
  command_line->AppendSwitchASCII("fingerprint_windows", tmp);
}
