...
std::vector<std::string> randomlyRemoveElements(std::vector<std::string> arr,
                                                unsigned int seed) {
  ...
  for (int i = 0; i < 8; ++i) {
    uint8_t byte = (seed >> (i * 8)) & 0xFF; 
    seedBytes.push_back(byte);
  }
  int i = 0;
...
int SSLClientSocketImpl::Init() {
  ...
  //    
  //    std::vector<std::string> stringsAarry = {":!aPSK", ":!kRSA", ":!ECDSA",
  //                                             ":!ECDSA+SHA1", ":!3DES"};
  //    
  //    int seed;
  //    std::istringstream(
